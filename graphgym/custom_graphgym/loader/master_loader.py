...
from custom_graphgym.transform.transforms import (pre_transform_in_memory,
                                           ...
                                           clip_graphs_to_size)
from custom_graphgym.ben_utils import add_k_hop_edges, add_k_leq_beta_adj, get_edge_labels
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if cfg.gnn.stage_type in multi_hop_stages:
        ...
        local_filedir = 'graphgym/custom_graphgym/loader/dataset/k_hop_datasets' # data location for verges/mac, local
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from .ogbg_code2_utils import idx2vocab, \
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from .dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from .dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from .dataset.peptides_structural import \
