...
# PyG 2.0.2 version -- uses optimizer_config
...
# PyG 2.2 version -- no more OptimConfig
def optimizer_adamW(params, base_lr, weight_decay):
...
# PyG 2.2
def scheduler_reduce_on_plateau(optimizer):
    if cfg.train.mode == 'standard':
        raise ValueError("ReduceLROnPlateau scheduler is not supported "
                        "by 'standard' graphgym training mode pipeline; "
                        "try setting config 'train.mode: custom'")
